...
argparser.add_argument("command", nargs="+", metavar="CMD", help="an mpd command")
...
argparser.add_argument(
    "--bind", default="", help="keybindings in a comma-separated list"
)
argparser.add_argument(
    "--sort", choices=MPD_FIELDS, metavar="MPD-TAG", help="tag field to sort items by"
)
argparser.add_argument(
    "--the-strip",
    action="store_true",
...
argparser.add_argument(
    "--dynamic-headers",
    choices=["yes", "no", "category"],
    default=views.NO_DYNAMIC_HEADERS,
    action=DynamicHeadersAction,
...
def run_with_args(view_func, mpc, cli_args):
    ...
    view_settings = views.ViewSettings(
        ...
        sort_field=cli_args.sort,
        the_strip=cli_args.the_strip,
    )
...
mfp_cmds.update(
    {
        "find": partial(run_with_args, views.singles_view),
